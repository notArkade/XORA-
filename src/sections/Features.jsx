    ...
    <section>
      <Element name="features">
        <div className="container">
          <div className="relative flex flex-nowrap border-2 border-s3 rounded-7xl md:flex-wrap md:overflow-hidden max-md:flex-col featuer-after md:g7 max-md:border-none max-md:rounded-none max-md:gap-3">
            ...
            <ul className="relative flex justify-around flex-grow px-[5%] border-2 border-s3 rounded-7xl max-md:hidden">
                ...
                {details.map(({id, icon, title}) => (
                    <li key={id} className="relative pt-16 pb-14 px-4">
                        ...
                        <div className="flex items-center justify-center mx-auto border-2 mb-3 border-s2 hover:border-s4 rounded-full size-20 transition-all duration-500 shadow-500">
                            <img src={icon} alt={title} className="size-17/20 object-contain z-20" />
                        </div>

                        <h3 className="relative z-2 max-w-36 base-small mx-auto my-0 text-center uppercase">{title}</h3>
                    </li>
                ))} 
